/* *****15.12.2015: customization for app-nea********
This is a custom release contains a patch for supporting a default date in the picker creation.
If you specify a default-date attribute with the format AAAA-MM-DD, the title of the view will start with the 
specified default date and not the current date. E.g. default-date="1995-12-25" */
	...
	var MomentPickerDirective = (function () {
		function MomentPickerDirective(timeout, sce, compile, document, momentPickerProvider) {
			...
			this.scope = {
				...
				minDate:   '=?',
				maxDate:   '=?',
                //add a default date for initializazion different from current date
                defaultDate: '@?'
			};
			$timeout     = timeout;
			$sce         = sce;
		...
		MomentPickerDirective.prototype.link = function ($scope, $element, $attrs) {
			...
			angular.forEach(['locale', 'format', 'minView', 'maxView', 'startView', 'leftArrow', 'rightArrow', 'defaultDate'], function (attr) {
			...
			$scope.limits = {
				...
				checkView: function () {
                    ...
                    if (!angular.isDefined($scope.view.defaultDate)) $scope.view.moment = moment($scope.defaultDate).locale($scope.locale);
